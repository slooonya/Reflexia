...
class InsightService:
  ...
  async def create_insight(data: InsightCreate, user_id: PydanticObjectId) -> InsightEntry:
    insight = InsightEntry(**data.model_dump(), user_id=user_id)
    await insight.insert()
    return insight
  

  async def get_insights(user_id: PydanticObjectId, period_type: str | None = None):
